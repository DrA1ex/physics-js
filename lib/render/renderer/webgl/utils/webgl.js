    ...
    for (const dc of dataConfig) {
        ...
        if (dc.textures) {
            for (const texConfig of dc.textures) {
                const params = texConfig.params ?? state._config.textures[texConfig.name];

                const slot = texConfig.slot;
                if (!Number.isInteger(slot) || slot < 0) throw new Error("Texture slot should be specified");

                gl.activeTexture(gl.TEXTURE0 + slot);
                if (texConfig.source) {
                    loadImageTexture(gl, state.textures[texConfig.name], texConfig.source, params);
                } else {
                ...
                gl.uniform1i(gl.getUniformLocation(program, texConfig.name), slot);
            }
        }
    }
}