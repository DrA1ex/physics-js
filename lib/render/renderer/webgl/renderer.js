...
import {SpriteObject, SpriteObjectLoader} from "./objects/sprite/object.js";
...
export class WebglRenderer extends IRenderer {
    ...
    /** @type {Map<ITextureSource, number>} */
    #textures = new Map();
    /** @type {Set<number>} */
    #freeTextureSlots = new Set();
    ...
    #prepare() {
        ...
        for (const texture of this.#textures.keys()) {
            ...
            SpriteObjectLoader.loadTexture(this.#gl, this.#configuration, texture, this.#getNextTextureSlot());
        }
    }

    #getNextTextureSlot() {
        const slot = this.#freeTextureSlots[Symbol.iterator]().next().value;
        if (!Number.isInteger(slot)) throw new Error("All texture slots are taken");

        this.#freeTextureSlots.delete(slot);
        return slot;
    }
    ...
    init() {
        ...
        const maxTextureSlots = this.#gl.getParameter(this.#gl.MAX_TEXTURE_IMAGE_UNITS);
    ...
    removeObject(obj) {
        if (this.#objectsByType.has(obj.key)) {
            ...
            if (obj instanceof SpriteObject) {
                this.#freeTexture(obj.texture);
            }
        }
    }

    /**
     * @param {ITextureSource} tex
     */
    #addTexture(tex) {
        const usageCount = this.#textures.get(tex) || 0
        this.#textures.set(tex, usageCount + 1);
    }

    #freeTexture(tex) {
        const usageCount = this.#textures.get(tex);
        if (usageCount === 1) {
            this.#freeTextureSlots.add(tex.slot);
            tex.slot = null;
        }
