...
import {Collider, Collision} from "./collision.js";
...
export class ImpulseBasedSolver {
    ...
    #collectCollisions() {
        ...
        this.#processTreeLeaf(this.stepInfo.tree.root, new Set());
    ...
    #processTreeLeaf(current, processedPairs) {
        if (current.leafs.length > 0) {
            for (let i = 0; i < current.leafs.length; i++) {
                ...
                for (let j = i + 1; j < current.leafs.length; j++) {
                    ...
                    if (Collider.isBoundaryCollide(l1.boundary, l2.boundary)) {
                        this.#processLeafOuterCollisions(l1, l2, processedPairs);
                    }
                }

                this.#processTreeLeaf(l1, processedPairs);
            }
        } else {
            this.#processLeafInnerCollisions(current, processedPairs);
        }
    }

    #processLeafOuterCollisions(leaf1, leaf2, processedPairs) {
        ...
        if (l1Empty && l2Empty) {
            for (const body1 of leaf1.items) {
                for (const body2 of leaf2.items) {
                    this.#processPairCollisions(body1, body2, processedPairs);
                }
            }
            ...
            for (const l1 of leaf1.leafs) {
                if (Collider.isBoundaryCollide(l1.boundary, leaf2.boundary)) {
                    this.#processLeafOuterCollisions(l1, leaf2, processedPairs);
                }
            }
    ...
    #processLeafInnerCollisions(leaf, processedPairs) {
    ...
    #processPairCollisions(body1, body2, processedPairs) {
        if (!body1.active && !body2.active) return;

        const id = this.#calcPairId(body1, body2);
        if (processedPairs.has(id)) {
            console.warn(`Pair ${id} already processed`);
            return;
        }
