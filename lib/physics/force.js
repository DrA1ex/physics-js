...
export class WindForce extends IForce {
    force;
    ...
    impulse(delta, body) {
        ...
        let start = Number.POSITIVE_INFINITY, end = Number.NEGATIVE_INFINITY;
        for (const p of body.points) {
            const projected = vector.dot(p);

            if (projected < start) start = projected;
            if (projected > end) end = projected;
        }

        const projectedSize = end - start;
        return {
