...
export class GlobalWind extends IForce {
    ...
    impulse(delta, body) {
        const vector = this.force.normalized();

        const {min, max} = GeomUtils.getProjectedInterval(vector.normalized(), body.points);
        const projectedSize = max - min;
        return {
            impulse: this.force.scaled(projectedSize * delta),
            point: body.position.copy()
        }
    }
}
...
export class SpotWindForce extends IForce {
    ...
    impulse(delta, body) {
        ...
        const perp = this.#normal.perpendicular();
        const {min, max} = GeomUtils.getProjectedInterval(perp, body.points);
        const projectedSize = max - min;

        const impulse = this.#normal.scaled(projectedSize / distance * this.#force * delta);
