...
/** @type{ObjectPool<BoundaryBox>} */
const BoundaryBoxPool = new ObjectPool(() => BoundaryBox.empty());
...
export class SpatialTree {
        ...
        for (let x = 0; x < this.divider; ++x) {
            ...
            for (let y = 0; y < this.divider; ++y) {
                ...
                if (filteredItems.length > 0) {
                    ...
                    BoundaryBoxPool.free(segmentBoundary);
                }
            }
        }
    ...
    free() {
        this.#freeLeaf(this.root);
    }

    #freeLeaf(current) {
        if (current.boundary !== current.segmentBoundary) BoundaryBoxPool.free(current.segmentBoundary);
        BoundaryBoxPool.free(current.boundary);

        current.boundary = null;
        current.segmentBoundary = null;
        current.items = null;
