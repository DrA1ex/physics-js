...
export class LineCollider extends PolygonCollider {
    static detectCollision(body1, body2) {
        ...
        const collisionCheck = GeomUtils.getProjectionIntersectionInfo(projection, points1, points2);

        if (!collisionCheck.result) {
            return new Collision(false);
        }

        const normal = PolygonCollider._alignNormal(body2, body1, projection);
        const collisionA = PolygonCollider._getCollisionPoint(points2, normal);
        const overlap = normal.dot(collisionA.delta(body1.position));
